...
const User = () => {
  ...
  const getDefaultImage = (propertyType) => {
    const imageMap = {
      ...
      'property': 'https://images.unsplash.com/photo-1560472354-b33ff0c44a43?w=400&h=300&fit=crop'
    };
    return imageMap[propertyType] || imageMap['property'];
  };
              ...
              {!isLoadingBookmarks && !bookmarksError && (
                <div className="user-bookmarks-grid">
                  {bookmarkedPredictions.length > 0 ? (
                    bookmarkedPredictions.slice(0, 3).map((property) => (
                      <div key={property.id} className="user-bookmark-card">
                        <div className="user-card-image">
                          <img 
                            src={getDefaultImage(property.property_type?.toLowerCase() || 'property')} 
                            alt={property.address} 
                          />
                        </div>
                        <div className="user-card-content">
                          <h3 className="user-card-address">{property.address}</h3>
                          <p className="user-card-date">Bookmarked on {formatDate(property.bookmarked_date)}</p>
                          <button 
