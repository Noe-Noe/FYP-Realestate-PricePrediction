...
const PricePrediction = () => {
  ...
  const handleSearch = async (e) => {
    ...
    if (userType === 'free') {
      if (!trialStatus || !trialStatus.trial_available) {
    ...
    // Use trial prediction if user is on free plan
    if (userType === 'free' && trialStatus?.trial_available) {
      try {
        await trialAPI.usePrediction();
        // Update trial status
        const updatedStatus = await trialAPI.checkStatus();
        setTrialStatus(updatedStatus);
      } catch (error) {
        console.error('Error using trial prediction:', error);
        setSearchError('Failed to use trial prediction. Please try again.');
        return;
      }
    }
  ...
  const handleTrialAccepted = async () => {
    try {
      await trialAPI.startTrial();
      const updatedStatus = await trialAPI.checkStatus();
      setTrialStatus(updatedStatus);
      // Retry the search after trial is accepted
      const searchEvent = { preventDefault: () => {} };
      await handleSearch(searchEvent);
    } catch (error) {
      console.error('Error starting trial:', error);
      setSearchError('Failed to start trial. Please try again.');
    }
  };
  ...
  return (
    <div className="user-dashboard">
      ...
      <TrialModal
        ...
        onTrialAccepted={handleTrialAccepted}
