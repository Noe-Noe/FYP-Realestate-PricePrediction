...
const Profile = () => {
  ...
  useEffect(() => {
    const loadUserProfile = async () => {
      try {
        ...
        try {
          const preferencesResponse = await authAPI.getUserPreferences();
          setPreferencesData({
            propertyTypes: preferencesResponse.propertyTypes || [],
            locations: preferencesResponse.locations || []
          });
        } catch (error) {
          console.error('Error loading preferences:', error);
          // Continue without preferences if they don't exist
        }
  ...
  const handleSavePreferences = async () => {
    try {
      ...
      await authAPI.saveUserPreferences(preferencesData);
    ...
    <div className="user-dashboard">
      ...
      <div className="user-dashboard-container">
        ...
        <main className="user-main-content">
          ...
          {/* User Preferences */}
          <section className="profile-section">
            <h2 className="profile-subsection-title">Property Preferences</h2>
            <p className="profile-section-description">
              Customize your property search preferences to get better recommendations and personalized results.
            </p>
            
            <div className="profile-preferences-content">
              {/* Property Types */}
              <div className="profile-preferences-group">
                <h3 className="profile-preferences-group-title">Preferred Property Types</h3>
                <div className="profile-preferences-options">
                  {[
                    'Office', 'Retail', 'Shop House', 'Business Parks',
                    'Single-user Factory', 'Multiple-user Factory', 'Warehouse'
                  ].map(type => (
                    <label key={type} className="profile-preferences-option">
                      <input
                        type="checkbox"
                        checked={preferencesData.propertyTypes.includes(type)}
                        onChange={() => handlePropertyTypeToggle(type)}
                        className="profile-preferences-checkbox"
                      />
                      <span className="profile-preferences-label">{type}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              {/* Locations */}
              <div className="profile-preferences-group">
                <h3 className="profile-preferences-group-title">Preferred Locations</h3>
                <div className="profile-preferences-options">
                  {[
                    'Central Region', 'East Region', 'North Region', 'Northeast Region',
                    'West Region', 'Downtown Core', 'Marina Bay', 'Orchard', 'Raffles Place'
                  ].map(location => (
                    <label key={location} className="profile-preferences-option">
                      <input
                        type="checkbox"
                        checked={preferencesData.locations.includes(location)}
                        onChange={() => handleLocationToggle(location)}
                        className="profile-preferences-checkbox"
                      />
                      <span className="profile-preferences-label">{location}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="profile-preferences-actions">
                <button
                  className="profile-save-preferences-btn"
